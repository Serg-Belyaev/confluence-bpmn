...
const MUTED_TYPES = [ //array of bpmn-element types. Comment lines below are to prohibit users for commenting the special bpmn-elements
...
function registerUnsavedChangesWarning() {
...
function loadScriptsAndStylesForEditor() {
    ...
    $("div.fullScreen_button").css("background-image", "url(/download/attachments/"+SOURCE_PAGE_ID+"/fullscreen.jpg)");
...
function loadScriptsAndStylesForViewer() {
    ...
    $("div.fullScreen_button_view_mode").css("background-image", "url(/download/attachments/"+SOURCE_PAGE_ID+"/fullscreen.jpg)");    
}
    ...
    function parseBPMNLinks() {
        ...
        $.each(elementRegistry._elements, function(index, elem) {
            ...
            try {
                ...
                for(var j = 0; j<extensionElements.values.length; j++) {
                    var a = extensionElements.values[j]; //elementRegistry structure depends on source object (viewer or modeler)??
                    if (a.$type.toLowerCase() != 'camunda:properties') continue;
                    var properties = a.values;
                    for (var i =0; i<properties.length; i++) {
                        if (properties[i].$type.toLowerCase() != 'camunda:property') continue;
                        if (properties[i].name.toLowerCase() == BPMN_PROPERTIES.url.toLowerCase()) {
                            var linkType = "";
                            switch (businessObject.$type) {
                                case "bpmn:UserTask": linkType = "processesAndTasks"; break 
...
function prepareElement(elem) {
    ...
    try {
        ...
        for (var j = 0; j< extensionElements.values.length;j++) {
            if (extensionElements.values[j].$type == 'camunda:properties') {
                var properties = extensionElements.values[j].$children; //scripts may be in first object of extension elements, properties - in the second.
                for (var i =0; i<properties.length; i++) {
                    if (properties[i].$type == 'camunda:property') {
